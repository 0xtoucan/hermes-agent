...
def validate_mcp_names(action: str, names: List[str]) -> Optional[str]:
    ...
    if action == "connect":
        ...
        if not_app:
            return (
                f"'connect' is a managed-connector action; MCP targets ({', '.join(not_app)}) use "
                "install, enable, authorize. Only a catalog entry hosted by a desktop application connects."
            )
        return None
