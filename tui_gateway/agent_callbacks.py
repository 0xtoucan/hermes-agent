...
def _agent_cbs(sid: str) -> dict:
    ...
    callbacks = {
        ...
        "read_terminal_callback": _read_ask("terminal.read.request", 30),
        "read_preview_callback": _read_ask("preview.read.request", 45),
        # drive_preview / annotate_preview (desktop GUI): same budget as the preview read it ends with.
        "drive_preview_callback": lambda payload: _ask("preview.act.request", sid, dict(payload), timeout=45),
        # read_window_below (desktop GUI): main process enumerates native windows.
        "read_window_below_callback": lambda: _ask("window.read.request", sid, {}, timeout=30),
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        val = _ask("secret.request", sid, pl)
        if not val:
    ...
    set_sudo_password_callback(lambda: _ask("sudo.request", sid, {}, timeout=120))
    ...
    def save_login_cb(origin, site):
        ...
        raw = _ask("vault.save_login.request", sid, {"origin": origin, "site": site}, timeout=180)
        try:
