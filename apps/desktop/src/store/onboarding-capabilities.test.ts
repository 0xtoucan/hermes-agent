...
// The gateway module is the RPC seam; this test pins the exact (connection, profile) request without opening a backend.
// An explicit object scope is a user pointing at a backend, so the dial is foreground (see capabilityScoped).
// oxlint-disable-next-line anti-slop/no-module-mocking
vi.mock('@/store/gateway', () => ({ activeGatewayProfileKey: () => 'default', requestGatewayForAgent }))
