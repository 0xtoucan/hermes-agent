...
describe('connection-request store', () => {
  ...
  it('never recomputes the deadline: status overlays the server value, updates leave it alone', () => {
    ...
    const connectedFrame = frame({ gmail: 'connected' }, { actor: 'backend_watcher', from: 'initiated', target: 'gmail', to: 'connected' })
    const updated = applyConnectionUpdate(overlaid, {
      ...connectedFrame,
      // A frame is the full snapshot; the row keeps its link only because the backend still sends it.
      targets: connectedFrame.targets.map(target => (target.name === 'gmail' ? { ...target, connect_url: 'https://l/gmail' } : target)),
