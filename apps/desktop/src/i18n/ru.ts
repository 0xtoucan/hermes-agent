...
export const ru = defineLocale({
  ...
  skills: {
    plugins: {
      consentLabel: 'Разрешения',
      consentPending: 'запрошены',
      consentAllowed: 'разрешены',
      consentPendingHint: 'Плагин запросил эти разрешения. Пока вы их не разрешите, он работает только с разрешениями по умолчанию, а всё сверх них блокируется.',
      consentAllowedHint: 'Вы разрешили эти разрешения для этого профиля. Отзыв возвращает плагин к разрешениям по умолчанию.',
      consentAllow: 'Разрешить',
      consentRevoke: 'Отозвать',
      consentAllowedToast: (name: string) => `${name} теперь может использовать запрошенные разрешения.`,
      consentRevokedToast: (name: string) => `${name} вернулся к разрешениям по умолчанию.`,
      consentRequestTitle: (name: string) => `${name} запрашивает разрешения`,
      consentRequestMessage: (count: number) => `Запрошено разрешений сверх стандартных: ${count}. Плагин работает без них, пока вы не разрешите их в «Возможности › Плагины».`,
      consentReview: 'Посмотреть',
      sandboxBlockedTitle: (name: string) => `Плагин «${name}» заблокирован`,
      sandboxBlockedPending: (action: string, method: string) => `Он попытался ${action} («${method}»), а вы этого ещё не разрешили. Проверьте его разрешения в «Возможности › Плагины».`,
      sandboxBlockedUndeclared: (action: string, method: string, capability: string) => `Он попытался ${action} («${method}») без возможности «${capability}». Плагин должен объявить её в desktop_capabilities своего plugin.yaml.`,
      sandboxDisabledTitle: (name: string) => `Плагин «${name}» отключён`,
      capabilityLabels: {
        ui: 'Добавлять интерфейс и показывать уведомления',
        storage: 'Хранить собственные настройки',
        events: 'Следить за событиями шлюза',
        rest: 'Обращаться к своему бэкенду',
        restAny: 'Вызывать любой маршрут бэкенда',
        gatewayRequest: 'Читать сессии, навыки и статус',
        promptSubmit: 'Отправлять запросы от вашего имени',
        llm: 'Выполнять запросы к модели',
        composer: 'Читать и редактировать ваш черновик',
        navigate: 'Переходить по разделам приложения',
        osClipboard: 'Записывать в буфер обмена',
        osDialogs: 'Открывать файловые диалоги',
        osOpenExternal: 'Открывать ссылки в браузере',
        osRevealPath: 'Показывать свои файлы в файловом менеджере',
        osNotify: 'Отправлять системные уведомления'
      }
    },
    tabSkills: 'Навыки',
