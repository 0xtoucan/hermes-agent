...
export const zhHant = defineLocale({
  ...
  skills: {
    plugins: {
      ...
      consentPending: '請求中',
      consentAllowed: '已允許',
      consentPendingHint: '此外掛請求了這些權限。在你允許之前，它只能使用預設權限，超出部分的操作會被阻止。',
      consentAllowedHint: '你已為此設定檔允許這些權限。撤銷後外掛恢復為預設權限。',
      consentAllow: '允許',
      consentRevoke: '撤銷',
      consentAllowedToast: (name: string) => `${name} 現在可以使用它請求的權限。`,
      consentRevokedToast: (name: string) => `${name} 已恢復為預設權限。`,
      consentRequestTitle: (name: string) => `${name} 請求權限`,
      consentRequestMessage: (count: number) => `它請求了 ${count} 項超出預設的權限。在你於「功能 › 外掛」中允許之前，它將在沒有這些權限的情況下執行。`,
      consentReview: '檢視',
      sandboxBlockedTitle: (name: string) => `外掛「${name}」已被阻止`,
      sandboxBlockedPending: (action: string, method: string) => `它試圖${action}（「${method}」），而你尚未允許。請在「功能 › 外掛」中檢視其權限。`,
      sandboxBlockedUndeclared: (action: string, method: string, capability: string) => `它試圖${action}（「${method}」），但沒有「${capability}」能力。外掛必須在其 plugin.yaml 的 desktop_capabilities 中宣告。`,
      sandboxDisabledTitle: (name: string) => `外掛「${name}」已停用`,
      capabilityLabels: {
        ui: '新增介面並顯示提示',
        storage: '保存自己的設定',
        events: '追蹤閘道事件',
        rest: '存取自己的後端',
        restAny: '呼叫任意後端路由',
        gatewayRequest: '讀取工作階段、技能和狀態',
        promptSubmit: '以你的身分送出提示',
        llm: '執行模型請求',
        composer: '讀取並編輯你的草稿',
        navigate: '在應用程式內導覽',
        osClipboard: '寫入剪貼簿',
        osDialogs: '開啟檔案對話框',
        osOpenExternal: '在瀏覽器中開啟連結',
        osRevealPath: '在檔案管理器中顯示其檔案',
        osNotify: '傳送系統通知'
      }
    },
    tabSkills: '技能',
