...
export const ja = defineLocale({
  ...
  skills: {
    plugins: {
      ...
      consentPending: '要求中',
      consentAllowed: '許可済み',
      consentPendingHint: 'このプラグインは次の権限を要求しています。許可するまではデフォルトの権限のみで動作し、それを超える操作はブロックされます。',
      consentAllowedHint: 'このプロファイルでこれらの権限を許可しています。取り消すとプラグインはデフォルトの権限に戻ります。',
      consentAllow: '許可',
      consentRevoke: '取り消す',
      consentAllowedToast: (name: string) => `${name} は要求した権限を使えるようになりました。`,
      consentRevokedToast: (name: string) => `${name} はデフォルトの権限に戻りました。`,
      consentRequestTitle: (name: string) => `${name} が権限を要求しています`,
      consentRequestMessage: (count: number) => `デフォルトを超える権限を ${count} 件要求しました。「機能 › プラグイン」で許可するまで、それらなしで動作します。`,
      consentReview: '確認',
      sandboxBlockedTitle: (name: string) => `プラグイン「${name}」をブロックしました`,
      sandboxBlockedPending: (action: string, method: string) => `「${action}」(${method}) を試みましたが、まだ許可されていません。「機能 › プラグイン」で権限を確認してください。`,
      sandboxBlockedUndeclared: (action: string, method: string, capability: string) => `「${action}」(${method}) を「${capability}」権限なしで試みました。プラグインは plugin.yaml の desktop_capabilities で宣言する必要があります。`,
      sandboxDisabledTitle: (name: string) => `プラグイン「${name}」を無効にしました`,
      capabilityLabels: {
        ui: 'UI を追加して通知を表示',
        storage: '自身の設定を保存',
        events: 'ゲートウェイのイベントを購読',
        rest: '自身のバックエンドと通信',
        restAny: '任意のバックエンドルートを呼び出す',
        gatewayRequest: 'セッション・スキル・状態を読み取る',
        promptSubmit: 'あなたとしてプロンプトを送信',
        llm: 'モデルリクエストを実行',
        composer: '下書きの読み取りと編集',
        navigate: 'アプリ内を移動',
        osClipboard: 'クリップボードに書き込む',
        osDialogs: 'ファイルダイアログを開く',
        osOpenExternal: 'ブラウザでリンクを開く',
        osRevealPath: 'ファイルマネージャーでファイルを表示',
        osNotify: 'システム通知を送信'
      }
    },
    tabSkills: 'スキル',
