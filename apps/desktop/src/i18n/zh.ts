...
export const zh = defineLocale({
  ...
  skills: {
    ...
    plugins: {
      ...
      consentPending: '请求中',
      consentAllowed: '已允许',
      consentPendingHint: '此插件请求了这些权限。在你允许之前，它只能使用默认权限，超出部分的操作会被阻止。',
      consentAllowedHint: '你已为此配置文件允许这些权限。撤销后插件恢复为默认权限。',
      consentAllow: '允许',
      consentRevoke: '撤销',
      consentAllowedToast: (name: string) => `${name} 现在可以使用它请求的权限。`,
      consentRevokedToast: (name: string) => `${name} 已恢复为默认权限。`,
      consentRequestTitle: (name: string) => `${name} 请求权限`,
      consentRequestMessage: (count: number) => `它请求了 ${count} 项超出默认的权限。在你于“功能 › 插件”中允许之前，它将在没有这些权限的情况下运行。`,
      consentReview: '查看',
      sandboxBlockedTitle: (name: string) => `插件“${name}”已被阻止`,
      sandboxBlockedPending: (action: string, method: string) => `它试图${action}（“${method}”），而你尚未允许。请在“功能 › 插件”中查看其权限。`,
      sandboxBlockedUndeclared: (action: string, method: string, capability: string) => `它试图${action}（“${method}”），但没有“${capability}”能力。插件必须在其 plugin.yaml 的 desktop_capabilities 中声明。`,
      sandboxDisabledTitle: (name: string) => `插件“${name}”已停用`,
      capabilityLabels: {
        ui: '添加界面并显示提示',
        storage: '保存自己的设置',
        events: '跟踪网关事件',
        rest: '访问自己的后端',
        restAny: '调用任意后端路由',
        gatewayRequest: '读取会话、技能和状态',
        promptSubmit: '以你的身份提交提示',
        llm: '运行模型请求',
        composer: '读取和编辑你的草稿',
        navigate: '在应用内导航',
        osClipboard: '写入剪贴板',
        osDialogs: '打开文件对话框',
        osOpenExternal: '在浏览器中打开链接',
        osRevealPath: '在文件管理器中显示其文件',
        osNotify: '发送系统通知'
      }
    },
    officialCatalog: '可安装',
